...
async def test_evolve_e2e_variant_merge_and_revert(tmp_path):
    ...
    proposal_id = engine.propose(skill_name="reader", current_body="Read files carefully.")
    await engine.merge(proposal_id, description="Use reader.")
    evolved = build_catalog([tmp_path / "skills"]).get("reader")
    await writer.revert(name="reader", version="v1")
    reverted = build_catalog([tmp_path / "skills"]).get("reader")

    assert "evolved pass_rate=1.000" in evolved.body
    assert "evolved pass_rate" not in reverted.body
